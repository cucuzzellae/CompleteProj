...
insecurity.education <- Food.insecurity.linked.to.obesity...Education
insecurity.income <- Food.insecurity.linked.to.obesity...Income
policy <- CDC_Nutrition__Physical_Activity__and_Obesity_._Legislation
time_obese_data <- time_obese_data
...
ui <- fluidPage(
  ...
  h3(id = "graphTitle", "Health policies over time"),
...
server <- function(input, output, session) {
  ...
  output$childAdult <- renderPlot ({
    percentData <- Percent.Adult.Obesity.vs..Childhood.Obesity.in.Different.States...Sheet2
  ...
  output$differentAges <- renderPlot({
    relativeRisk <- `Relative.risk.of.overweight.and.obesity.(owt).at.each.age.according.to.parental.weight.status...Sheet1`
  ...
  output$interactiveChild <- renderPlot({
    print(input$double)
    physicallyInactive <- PhysicallyInactive...Sheet2
  ...
  output$infancy <- renderPlot ({
    seData <- Socioeconomic.status.and.weight.gain.in.early.infancy...Sheet1
    print(input$ses)
    if (input$ses == "1") (
  ...
  sportsTeams <- Public.Sports.Teams...PublicSportsTeams
  output$v <- renderText({
  ...
  mentalHealthBoys <- Mental.Health.vs..Obesity...Boys
  mentalHealthGirls <- Mental.Health.vs..Obesity...Girls
